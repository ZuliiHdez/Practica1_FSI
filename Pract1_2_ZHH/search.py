...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            path=node.path()
            return print("Nodos Generados: ", generate_nodes,"\nNodos Visitados: ", visited_nodes,"\nCosto total: ", node.path_cost,"\nRuta: ", path)

        if node.state not in closed:
            closed[node.state] = True
            successors = node.expand(problem)
            fringe.extend(successors)
            generate_nodes += len(successors)

    return print("Nodos Generados: ", generate_nodes,"\nNodos Visitados: ", visited_nodes,"\nCosto total: ", None,"\nRuta: ", None) # No solution found
...
def branch_and_bound(problem):
    ...
    queue = [(start_node.path_cost, start_node)]  # Lista para actuar como cola de prioridad

    while queue:
        queue.sort(key=lambda x: x[0])  # Ordenar por costo ascendente
        _, node = queue.pop(0)
        visited_nodes += 1

        if problem.goal_test(node.state):
            path = node.path()
            print("Nodos Generados: ", generated_nodes, "\nNodos Visitados: ", visited_nodes, "\nCosto total: ", node.path_cost, "\nRuta: ", path)
            return

        if node.state not in visited:
            visited.add(node.state)
            expansion = node.expand(problem)
            generated_nodes += len(expansion)

            for child in expansion:
                queue.append((child.path_cost, child))
...
def branch_and_bound_with_heuristic(problem):
    ...
    visited = set()
    start_node = Node(problem.initial)
    heuristic = start_node.path_cost + problem.h(start_node)
    queue = [(heuristic, start_node)]  # Lista para actuar como cola de prioridad
    ...
    while queue:
        ...
        if node.state not in visited:
            ...
            for child in expansion:
                child_heuristic = child.path_cost + problem.h(child)
                queue.append((child_heuristic, child))
